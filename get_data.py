...
def preprocess_feature(x, feature_dim):
    """
    Feature is the adc values; scale it such that each value is between 0 and 1.
    """
    x_max = np.max(x)
    x = x/x_max
    return x.reshape(1, *feature_dim)

def preprocess_label(y, num_classes, label_dim):
    return np_utils.to_categorical(y, num_classes=num_classes).reshape(1, *label_dim)
